...
def problem_1p (X):
    ...
    x_repeat = np.repeat(x_3d, X.shape[-1], axis = 2)
    x_swap = np.swapaxes(np.repeat(x_3d, X.shape[-1], axis = 2), axis1 = 1, axis2 = 2)
    return np.sqrt(np.sum((x_repeat - x_swap)**2, axis = 0))

# %%

def linear_regression (X_tr, y_tr):
    X_tr = X_tr.T     # Since we consider X as a column vector! 
...
def train_age_regressor ():
    ...
    print("Train FSME is: {:.4f}".format(fMSE(X_tr, w, ytr))) # 50.4653
    print("Test FSME is: {:.4f}".format(fMSE(X_te, w, yte)))  # 268.7447
# %%
